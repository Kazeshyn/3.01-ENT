...
<html lang="fr">
    ...
    <main>
    <p><a id="retour" href="reserv_materiel.php">Retour au formulaire</a></p>
    <h1>Réservations matérielles faites </h1>

    <section>
        <?php
            $requete = "SELECT * FROM reservation_materiel ORDER BY date_debut_m DESC";
            ...
            foreach ($result as $reserv) { 
                ...
                if ($stmt_utilisateur->rowCount() > 0) {
                    ...
                    $id_materiel = $reserv["id_materiel"];
                    $requete_mat = "SELECT * FROM materiel WHERE id_materiel = $id_materiel";
                    $stmt_mat = $pdo->prepare($requete_mat);
                    $stmt_mat->execute();
            
                    // Vérifiez si la requête a réussi avant d'accéder aux résultats
                    if ($stmt_mat->rowCount() > 0) {
                        $info_mat = $stmt_mat->fetch();
            ?>
                        <article>
                            <div class="text">
                                <h2><?= $info_user["nom"] ?>  <?= $info_user["prenom"] ?></h2>
                                <p><i><?= $reserv["date_debut_m"] ?></i></p>
                                <p><i><?= $reserv["horaire_debut_m"] ?></i></p>
                                <p><i><?= $reserv["date_fin_m"] ?></i></p>
                                <p><i><?= $reserv["horaire_fin_m"] ?></i></p>
                                <p><?= $info_mat["nom_materiel"] ?> </p>
                            </div>
                        </article>
            <?php
                    } else {
